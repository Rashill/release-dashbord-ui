...
import { ConfigService, Schema } from './config.service';
...
export class RDTableComponent implements OnInit {
  ...
  constructor(private modalService: NgbModal, httpClient:  HttpClient) {
    ...
    if(this.opt_obj.loadURL != undefined || this.opt_obj.apiURL !=undefined){
      this.load().subscribe((data) => this.data);
    }
  }
 ...
 */
  ...
  createRow(row) {
    ...
    if(this.opt_obj.createURL != undefined){
      ...
      this.httpClient.post(`${this.opt_obj.apiURL}/${this.opt_obj.name}/`, row);
    }
  }
  ...
  updateRow(rowIndex, row) {
    ...
    if(this.opt_obj.updateURL != undefined){
      ...
      this.httpClient.put(`${this.opt_obj.apiURL}/${this.opt_obj.name}/${this.data[rowIndex][this.opt_obj.pKey]}`, row);
    }
  }
  ...
  deleteRow(rowIndecies: number[]) {
    ...
    for(let rowIndex of rowIndecies){
      if(this.opt_obj.deleteURL != undefined){
        ...
        this.httpClient.delete(`${this.opt_obj.apiURL}/${this.opt_obj.name}/${this.data[rowIndex][this.opt_obj.pKey]}`);
      }
    }
  }
  ...
  load(){
    if(this.opt_obj.loadURL != undefined){
       return this.httpClient.get(`${this.opt_obj.loadURL}`);
    }else if(this.opt_obj.apiURL != undefined){
      return this.httpClient.get(`${this.opt_obj.apiURL}/${this.opt_obj.name}/`);
    }
  }
